...
export default class FeatureArtGallery extends Feature
{
    ...
    async loadArtwork()
    {
        ...
        let txlist = await etherscan.account.txlist('0xb932a70a57673d89f4acffbe830e8ed7f75fb9e0', 1, 'latest', 1, 100, 'asc');
        ...
        for(let txn of txlist.result)
        {
            ...
            if(input.name === 'addNewToken')
            {
                ...
                this.createPainting(data, type.mime)
            }
        }
    }
    ...
    createPainting(data, type)
    {
        let dataURI = "data:" + type + ";base64," + this._arrayBufferToBase64(data)
        ...
        let mesh = new THREE.Mesh(new THREE.PlaneBufferGeometry(1, 1), new THREE.MeshBasicMaterial({ map:texture }))
        mesh.position.set(this.pieces.length * 2, 0, -10)
